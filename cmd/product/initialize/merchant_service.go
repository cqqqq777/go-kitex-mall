...
func InitMerchant() merchant.Client {
	// init resolver
	...
	r := nacos.NewNacosResolver(nacosCli, nacos.WithGroup(consts.MerchantGroup))
	if err != nil {
	...
	// create a new client
	c, err := merchant.NewClient(
		config.GlobalServerConfig.MerchantSrvInfo.Name,
		client.WithResolver(r),                                     // service discovery
		client.WithLoadBalancer(loadbalance.NewWeightedBalancer()), // load balance
		client.WithMuxConnection(1),                                // multiplexing
		client.WithSuite(tracing.NewClientSuite()),
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: config.GlobalServerConfig.MerchantSrvInfo.Name}),
	)
	if err != nil {
		log.Zlogger.Fatalf("cannot init merchant client err:%s", err.Error())
	}
	return c
}
